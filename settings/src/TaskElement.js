...
const TaskElement = ({notice, onCloseTaskHandler, index}) => {
    const { highLightField } = useContext(DashboardContext);

    const handleClick = () => {
        highLightField(notice.output.highlight_field_id);
    }
    ...
    return(
        <div className="rsssl-task-element">
            ...
            {notice.output.highlight_field_id && <span className="rsssl-task-enable" onClick={handleClick}>{__("Enable", "really-simple-ssl")}</span> }
