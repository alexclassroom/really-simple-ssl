...
const Header = ({fields}) => {
    const {selectedMainMenuItem, setSelectedMainMenuItem} = useContext(DashboardContext);
    ...
    const handleClick = (menuId) => {
        setSelectedMainMenuItem(menuId);
    }

    const menu = rsssl_settings.menu;
    const plugin_url = rsssl_settings.plugin_url;
    const knowledgeBaseBtnClass = !rsssl_settings.pro_active ?'button button-black' :'';
    return (
            ...
            <div className="rsssl-header">
                ...
                <div className="rsssl-header-left">
                    <nav className="rsssl-header-menu">
                        <ul>
                            {menu.map((menu_item, i) => <li key={i}><a className={ selectedMainMenuItem === menu_item.id ? 'active' : '' } onClick={ () => handleClick(menu_item.id) } href={"#" + menu_item.id.toString()} >{menu_item.label}</a></li>)}
                        </ul>
                    </nav>
                </div>
